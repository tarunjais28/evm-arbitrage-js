...
class SwapData extends DerivedData {
  ...
  async getReserveBalances(): Promise<void> {
  ...
  async calcPostReserves(): Promise<void> {
  ...
  async _swap(amounts: bigint[]): Promise<void> {
  ...
  async _swapSupportingFeeOnTransferTokens(): Promise<void> {
    for (let i = 0; i < this.path.length - 1; i++) {
      const input = this.path[i];
      ...
      const pair = await getPairAddress(input, output);
      if (!pair) continue;
      ...
      const [reserveInput, reserveOutput] =
        input.toLowerCase() === token0.toLowerCase()
          ? [reserve0, reserve1]
          : [reserve1, reserve0];
  ...
  async swap(): Promise<void> {
    const functionHandlers: { [key: string]: () => Promise<void> } = {
